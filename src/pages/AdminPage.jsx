  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-4 bg-gray-100 p-0.5 rounded-lg inline-flex text-sm">
        {['overview','users','articles'].map(t=><button key={t} onClick={()=>setTab(t)} className={`px-3 py-1.5 rounded-md ${tab===t?'bg-[var(--win-card)] text-[var(--win-text)] shadow font-medium':'text-gray-600'}`}>{t==='overview'?'概览':t==='users'?'用户':'文章'}</button>)}
      </div>
      ...
      {tab==='users'&&<div className="card overflow-hidden"><table className="w-full text-sm"><thead><tr className="border-b"><th className="text-left p-3 text-gray-500">用户</th><th className="text-left p-3 text-gray-500">角色</th><th className="text-left p-3 text-gray-500">注册</th><th className="text-right p-3 text-gray-500">操作</th></tr></thead><tbody>{users.map(u=><tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50"><td className="p-3"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold">{u.displayName?.[0]}</div><div><p className="font-medium text-xs">{u.displayName}</p><p className="text-xs text-gray-400">@{u.username}</p></div></div></td><td className="p-3"><select value={u.role} onChange={e=>handleRole(u.id,e.target.value)} className={`text-xs px-2 py-0.5 rounded-full border ${roleColor(u.role)} border-[var(--win-border)] bg-[var(--win-card)]`} disabled={u.id===user.id}><option value="user">用户</option><option value="author">作者</option><option value="admin">管理员</option></select></td><td className="p-3 text-xs text-gray-500">{new Date(u.createdAt).toLocaleDateString('zh-CN')}</td><td className="p-3 text-right">{u.id!==user.id&&<button onClick={()=>handleDelUser(u.id)} className="text-red-500"><Trash2 size={14}/></button>}</td></tr>)}</tbody></table></div>}
