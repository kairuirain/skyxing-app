...
export default function AdminPage() {
  ...
  const loadData = async () => {
    try {
      const [s, u, a] = await Promise.all([
        api.getStats(),
        api.getAdminUsers(),
        api.getAdminArticles(sortBy, sortOrder),
      ]);
      setStats(s.stats);
  ...
  const handleWeightChange = async (aid, weight) => {
    const n = parseInt(weight, 10);
    if (isNaN(n)) return;
    try { await api.updateArticleWeight(aid, n); setEditingWeight(null); loadData(); } catch(e) { alert(e.message); }
  };
  ...
  const SortHeader = ({ field, label }) => (
    <th className="text-left p-3 text-gray-500 cursor-pointer hover:text-gray-700 select-none" onClick={() => toggleSort(field)}>
      <span className="flex items-center gap-1">
        {label}
        {sortBy === field ? (sortOrder === 'desc' ? <ArrowDown size={12} /> : <ArrowUp size={12} />) : null}
      </span>
    </th>
  );
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-4 bg-gray-100 p-0.5 rounded-lg inline-flex text-sm">
        {['overview','users','articles'].map(t=><button key={t} onClick={()=>setTab(t)} className={`px-3 py-1.5 rounded-md ${tab===t?'bg-white text-gray-900 shadow font-medium':'text-gray-600'}`}>{t==='overview'?'概览':t==='users'?'用户':'文章'}</button>)}
      </div>

      {tab==='overview'&&stats&&<div className="grid grid-cols-2 gap-3 mb-5">
      ...
      {tab==='articles'&&<div className="card overflow-hidden">
        <div className="px-3 py-2 border-b border-gray-100 flex items-center gap-2 text-xs text-gray-500">
          <span>排序：</span>
          {[{f:'createdAt',l:'发布时间'},{f:'views',l:'阅读量'},{f:'weight',l:'权重'}].map(({f,l})=>(
            <button key={f} onClick={()=>toggleSort(f)} className={`px-2 py-0.5 rounded ${sortBy===f?'bg-primary-100 text-primary-700 font-medium':'hover:bg-gray-100'}`}>
              {l} {sortBy===f?(sortOrder==='desc'?'↓':'↑'):''}
            </button>
          ))}
          <span className="ml-auto text-gray-400">TIP: 按权重排序后可编辑数值自定义顺序</span>
        </div>
        <table className="w-full text-sm"><thead><tr className="border-b">
          <SortHeader field="weight" label="权重" />
          <th className="text-left p-3 text-gray-500">标题</th>
          <th className="text-left p-3 text-gray-500">作者</th>
          <SortHeader field="createdAt" label="发布时间" />
          <SortHeader field="views" label="阅读" />
          <th className="text-left p-3 text-gray-500">状态</th>
          <th className="text-right p-3 text-gray-500">操作</th>
          ...
          <tr key={a.id} className="border-b border-gray-50 hover:bg-gray-50">
            <td className="p-3">
              <span className="flex items-center gap-1 text-xs">
                ...
                {editingWeight === a.id ? (
                  <input type="number" defaultValue={a.weight||0} onBlur={e=>handleWeightChange(a.id,e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleWeightChange(a.id,e.target.value)} className="w-14 input text-xs py-0.5 px-1" autoFocus />
                ) : (
                  <span className="cursor-pointer hover:text-primary-600" onClick={() => setEditingWeight(a.id)} title="点击修改权重">{a.weight||0}</span>
                )}
              </span>
            </td>
            <td className="p-3 text-xs font-medium truncate max-w-[180px]">{a.title}</td>
            <td className="p-3 text-xs text-gray-500">{a.author?.displayName}</td>
            <td className="p-3 text-xs text-gray-500">{new Date(a.createdAt).toLocaleDateString('zh-CN')}</td>
            <td className="p-3 text-xs text-gray-500">{a.views||0}</td>
            <td className="p-3"><span className={`text-xs px-1.5 py-0.5 rounded-full ${a.status==='published'?'bg-green-100 text-green-700':'bg-yellow-100 text-yellow-700'}`}>{a.status==='published'?'发布':'草稿'}</span></td>
            <td className="p-3 text-right"><button onClick={()=>handleDelArticle(a.id)} className="text-red-500"><Trash2 size={14}/></button></td>
          </tr>
        ))}</tbody></table></div>}
    </div>
  );
