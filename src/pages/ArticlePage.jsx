    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="border-t pt-5">
        ...
        <div className="space-y-3">
          {topComments.map(c => <CommentItem key={c.id} comment={c} replies={getReplies(c.id)} currentUser={user} onReply={setReplyTo} onDelete={handleDeleteComment} formatDate={formatDate} />)}
    ...
    <div className="border-l-2 border-gray-100 pl-3">
      <div className="flex gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5"><span className="font-medium text-xs">{comment.user?.displayName}</span><span className="text-xs text-gray-400">{formatDate(comment.createdAt)}</span></div>
          ...
          {replies.length > 0 && <div className="mt-2 space-y-2">{replies.map(r => <CommentItem key={r.id} comment={r} replies={[]} currentUser={currentUser} onReply={onReply} onDelete={onDelete} formatDate={formatDate}/>)}</div>}
        </div>
      </div>
    </div>
  );
